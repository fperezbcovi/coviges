 ...
 * @since    0.5
 */
add_action( 'wp_enqueue_scripts', 'naxos_enqueue_scripts' );
function naxos_enqueue_scripts() {
    wp_enqueue_script( 'simple-likes-public-js', plugins_url( 'simple-likes-public.js', __FILE__ ), array( 'jquery' ), '0.5', false );
    wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
        'ajaxurl' => admin_url( 'admin-ajax.php' ),
        'like' => esc_html__( 'Like', 'naxos' ),
        'unlike' => esc_html__( 'Unlike', 'naxos' )
    ) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_process_simple_like', 'naxos_process_simple_like' );
add_action( 'wp_ajax_process_simple_like', 'naxos_process_simple_like' );
function naxos_process_simple_like() {
    // Security
    $nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
    if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
        exit( esc_html__( 'Not permitted', 'naxos' ) );
    }
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
    ...
    if ( already_liked( $post_id, $is_comment ) ) {
        ...
        $title = esc_html__( 'Unlike', 'naxos' );
        $icon = $icon_full;
    } else {
        $class = '';
        $title = esc_html__( 'Like', 'naxos' );
        $icon = $icon_empty;
    }
...
function get_liked_icon() {
    ...
    $icon = '<span class="sl-icon"><svg role="img" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0" y="0" viewBox="0 0 128 128" enable-background="new 0 0 128 128" xml:space="preserve"><path class="heart-full" d="M124 20.4C111.5-7 73.7-4.8 64 19 54.3-4.9 16.5-7 4 20.4c-14.7 32.3 19.4 63 60 107.1C104.6 83.4 138.7 52.7 124 20.4z"/></svg></span>';
    return $icon;
} // get_liked_icon()
...
// User Profile List
add_action( 'show_user_profile', 'naxos_show_user_likes' );
add_action( 'edit_user_profile', 'naxos_show_user_likes' );
function naxos_show_user_likes( $user ) { ?>
    <table class="form-table">
        <tr>
            <th><label for="user_likes"><?php esc_html_e( 'You Like:', 'naxos' ); ?></label></th>
            <td>
                <?php
                ...
                <?php else : ?>
                    <p><?php esc_html_e( 'You do not like anything yet.', 'naxos' ); ?></p>
                    <?php
                endif;
                wp_reset_postdata();
                ?>
