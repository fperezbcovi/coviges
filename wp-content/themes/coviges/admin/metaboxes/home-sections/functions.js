function naxos_filter_params( ) {
	jQuery( '[data-header-section]' ).hide( );
	jQuery( '[data-header-section="' + jQuery( '#header-section' ).val( ) + '"]' ).show( );
}

function naxos_slides_content( ) {
	if ( naxosSlidesContent.length > 0 ) {
		jQuery.each( naxosSlidesContent, function( that, node ) {
...
jQuery( document ).ready( function( jQuery ) {	
	jQuery( '.meta-item-upload' ).on( 'click', function( evt ) {
		evt.preventDefault( );
		...
		frame = wp.media( {
			title: naxos_home_lng.insert_media,
			frame: 'post',
			multiple: is_multiple,
			library: { type: 'image' },
			button: { text: naxos_home_lng.insert_media },
		} );
			...
			if ( ! is_multiple ) jQuery( area ).val( imageArray.join( ', ' ) );
				...
				if ( area_id == '#slideshow-fields' ) {
					...
					images.each( function( image ) {
						...
						'  ' + naxos_home_lng.image + ' <span class="meta-item-row-alt-c">-</span>' +
						'  <input type="text" class="meta-item-l alt" name="slideshow-alt-images[]" value="' + image.attributes.url + '">' +
						'  <input type="button" class="button" data-remove-image="true" value="' + naxos_home_lng.remove + '">' +
						'</div>';
