<?php
// Breadcrumbs
if ( ! function_exists( 'naxos_breadcrumbs' ) ) {
	function naxos_breadcrumbs( ) {
		global $post, $naxos_config;

		$text['home']     = ( ! empty( $naxos_config['home-page-title'] ) ? $naxos_config['home-page-title'] : esc_html__( 'Home', 'naxos' ) );
		$text['category'] = '%s';
		$text['search']   = esc_html__( 'Search Results for Query &ndash; %s', 'naxos' );
		$text['tag']      = esc_html__( 'Posts Tagged &ndash; %s', 'naxos' );
		$text['author']   = esc_html__( 'Articles Posted by %s', 'naxos' );
		$text['404']      = esc_html__( 'Error 404', 'naxos' );
		$text['page']     = esc_html__( 'Page', 'naxos' );
